create schema chat;
grant usage on schema chat to ta_get,ta_post;
...
create function range(startid bigint, endid bigint, lim integer)
  ...
  select chat_id,account_id,account_image_url,chat_reply_id,chat_markdown,chat_at,chat_change_id
       ...
       , chat_is_first
       , chat_is_last
       , (select to_jsonb(array_agg(array[encode(chat_onebox_hash,'hex'),chat_onebox_markdown])) from chat_onebox o where o.chat_id=c.chat_id)
  from c
...
--
--
create function new(msg text, replyid integer, pingids integer[], hashes bytea[], markdown text[]) returns bigint language sql security definer set search_path=db,api,pg_temp as $$
  ...
  with i as (insert into chat(community_id,room_id,account_id,chat_markdown,chat_reply_id)
    ...
    , ob as (insert into chat_onebox(chat_id,chat_onebox_hash,chat_onebox_markdown) select chat_id,unnest(hashes),unnest(markdown) from i)
