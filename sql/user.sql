create schema usr;
grant usage on schema usr to get;
...
select community_id
      ...
      ,kind_description,kind_has_question_votes
from api._question natural join db.question natural join db.kind
...
from (select account_id,account_name,account_derived_name from db.account natural join api._account where account_id=get_user_id()) u
     cross join (select community_id,community_name,community_display_name
                              ,community_rgb_dark,community_rgb_mid,community_rgb_light,community_rgb_highlight,community_rgb_warning
