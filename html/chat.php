...
if(isset($_GET['activeusers'])){
  foreach(db("select account_id,account_name,account_is_me
    ...
    <img title="<?=($account_name)?$account_name:'Anonymous'?> (Reputation: <?=$account_community_votes?>)" class="active-user<?=($account_is_me==='t')?' me':''?>" data-id="<?=$account_id?>" data-name="<?=explode(' ',$account_name)[0]?>" src="/identicon.php?id=<?=$account_id?>"><?
  }
  exit;
}
