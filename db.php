...
$connection = pg_connect("host='cluster1.cluster-c8l1itv3i2dg.eu-west-2.rds.amazonaws.com' dbname=ta user=".$user." password=".$user." sslmode=require") or fail(403);
function db($query,...$params) {
  ...
  for($i = 0; $i<pg_num_fields($res); $i++){
    for($j = 0; $j<pg_num_rows($res); $j++){
      if(pg_field_is_null($res,$j,pg_field_name($res,$i))){
        $rows[$j][pg_field_name($res,$i)] = null;
      }else{
