...
?>
<div id="starboard-wrapper">
  <?if(ccdb("select count(*)>0 from chat where chat_star_count>0 limit 10")){?>
    <div class="label">Starred:</div>
    <div id="starboard">
      <?foreach(db("select * from chat limit 10") as $r){ extract($r);?>
        <div id="s<?=$chat_id?>" class="message" data-id="<?=$chat_id?>" data-name="<?=$account_name?>" data-reply-id="<?=$chat_reply_id?>">
          <span class="who" title="<?=($chat_account_is_me?'Me':$account_name)?><?=$chat_reply_id?' replying to '.($chat_reply_account_is_me?'Me':$chat_reply_account_name):''?>">
            ...
            <?=$chat_reply_id?'<a href="#c'.$chat_reply_id.'" style="color: #'.$colour_dark.'; text-decoration: none;">&nbsp;replying to&nbsp;</a> '.($chat_reply_account_is_me?'<em>Me</em>':$chat_reply_account_name):''?>
