...
<?foreach(db("select * from chat limit 30") as $r){ extract($r);?>
  <div id="s<?=$chat_id?>" class="message" data-id="<?=$chat_id?>" data-name="<?=$account_name?>" data-reply-id="<?=$chat_reply_id?>">
    ...
    <span class="buttons">
      ...
      <span class="button-group show">
        <i class="stars <?=$chat_i_starred?'me ':''?>fa fa-fw fa-star<?=($room_can_chat&&!$chat_i_starred&&!$chat_account_is_me)?'-o':''?>" data-count="<?=$chat_star_count?>"></i>
        <i></i>
        <a href="/transcript?room=<?=$room_id?>&id=<?=$chat_id?>#c<?=$chat_id?>" class="fa fa-fw fa-link" title="permalink"></a>
        <i></i>
      </span>
    </span>
  </div>
