<?
header("Content-Security-Policy: default-src 'none'; script-src 'self'; style-src 'self' 'unsafe-inline'; style-src-elem 'self'; style-src-attr 'unsafe-inline'; img-src * data:; font-src 'self'; connect-src 'self' tio.run dbfiddle.uk post.topanswers.xyz; form-action 'self' post.topanswers.xyz stackoverflow.com;");
require '../../../config.php';
require '../../../db.php';
require '../../../nocache.php';
require '../../../hash.php';
    ...
    <fieldset id="activity">
      ...
      <fieldset>
        ...
        <table class="data" data-order='[[0,"desc"]]' data-page-length='10'>
          <thead>
            <tr><th>date/time</th><th>type</th><th>status</th><th>title</th><th>stars</th><th>answers</th></tr>
          </thead>
          <tbody>
            <?foreach(db("select question_id,question_title,question_is_draft,question_votes,sanction_description,question_answers
