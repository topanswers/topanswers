...
select community_id,community_name,community_room_id,community_dark_shade,community_mid_shade,community_light_shade from db.community where community_name<>'meta' or current_setting('custom.uuid',true)::uuid is not null;
--
...
--
create function new_chat(luuid uuid, roomid integer, msg text) returns bigint language sql security definer set search_path=db,world,pg_temp as $$
...
create function change_account_name(nname text) returns void language sql security definer set search_path=db,world,pg_temp as $$
  select _error('invalid username') where nname is not null and not nname~'^[A-Za-zÀ-ÖØ-öø-ÿ][ 0-9A-Za-zÀ-ÖØ-öø-ÿ]*[0-9A-Za-zÀ-ÖØ-öø-ÿ]$';
