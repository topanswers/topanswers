...
create function _new_question(aid integer, title text, markdown text, tags text, seqid integer, seat timestamptz) returns integer language sql security definer set search_path=db,api,pg_temp as $$
  ...
  with r as (insert into room(community_id) values(get_community_id()) returning community_id,room_id)
     , q as (insert into question(community_id,account_id,kind_id,question_type,question_at,question_title,question_markdown,question_room_id,license_id,codelicense_id,question_se_question_id,question_se_imported_at)
             select community_id,aid,1,'question',seat,title,markdown,room_id,4,1,seqid,current_timestamp from r returning question_id)
